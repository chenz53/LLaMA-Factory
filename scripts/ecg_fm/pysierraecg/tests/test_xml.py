...
def test_read_file(
    filename: str, expected_doc_type: str, expected_doc_ver: str
) -> None:
...
def test_assert_leads(
    filename: str, expected_leads: int, expected_midpoints: List[int]
) -> None:
    f = read_file(filename)
...
def test_no_repbeats() -> None:
    ...
    f = read_file(
        "tests/fixtures/1_04_01/2020-5-18_15-48-11.xml", include_repbeats=False
    )
    assert len(f.repbeats) == 0
